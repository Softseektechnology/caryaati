...
const OptionsStep = () => {
  const { selectedCars, setSelectedCars } = CaryaatiContext(); // CHANGED: Use selectedCars

  const carPlaceholder = '/images/model_sp_596_639.webp';
  const sameVendorCars: Car[] = [
    { name: 'Ford Fiesta', year: 2019, category: 'Sedan', image: carPlaceholder, specs: { luggage: 3, transmission: 'Auto', ac: true, passengers: 5 }, deposit: 1000, rent: 1800 },
    { name: 'Kia Rio', year: 2019, category: 'Hatchback', image: carPlaceholder, specs: { luggage: 2, transmission: 'Auto', ac: true, passengers: 4 }, deposit: 1000, rent: 1950 },
    { name: 'Nissan Sentra', year: 2019, category: 'Sedan', image: carPlaceholder, specs: { luggage: 3, transmission: 'Auto', ac: true, passengers: 5 }, deposit: 1000, rent: 2100 },
    { name: 'Kia Forte', year: 2020, category: 'Sedan', image: carPlaceholder, specs: { luggage: 2, transmission: 'Auto', ac: true, passengers: 5 }, deposit: 1000, rent: 2250 },
  ];
  const differentVendorCars: Car[] = [
    { name: 'Toyota RAV4', year: 2023, category: 'SUV', image: carPlaceholder, specs: { luggage: 3, transmission: 'Auto', ac: true, passengers: 5 }, deposit: 2000, rent: 112.50 },
    { name: 'Nissan Sunny', year: 2022, category: 'Sedan', image: carPlaceholder, specs: { luggage: 3, transmission: 'Auto', ac: true, passengers: 5 }, deposit: 1000, rent: 1260 },
    { name: 'Kia Pegas', year: 2022, category: 'Sedan', image: carPlaceholder, specs: { luggage: 2, transmission: 'Auto', ac: true, passengers: 5 }, deposit: 1000, rent: 1350 },
    { name: 'Hyundai Accent', year: 2020, category: 'Sedan', image: carPlaceholder, specs: { luggage: 3, transmission: 'Auto', ac: true, passengers: 5 }, deposit: 1000, rent: 1350 },
  ];

  const handleSelect = (car: Car) => {
    // CHANGED: Toggle car selection
    setSelectedCars(prev => {
      const isSelected = prev.some(c => c.name === car.name && c.year === car.year);
      if (isSelected) {
        // Remove car if already selected
        return prev.filter(c => !(c.name === car.name && c.year === car.year));
      } else {
        // Add car to selected list
        return [...prev, car];
      }
    });
  };

  const CarOptionCard = ({ car }: { car: Car }) => {
    const isSelected = selectedCars.some(c => c.name === car.name && c.year === car.year); // CHANGED: Check array
    ...
    return (
      <div
        className={`border rounded-lg overflow-hidden shadow-lg transition-transform hover:scale-105 bg-white ${
          isSelected ? 'border-blue-600 bg-blue-50' : ''
        }`}
      >
        <div className="relative h-40 w-full">
          <Image src={car.image} alt={`${car.name} ${car.year}`} layout="fill" objectFit="cover" />
          <span className="absolute top-2 left-2 bg-blue-600 text-white text-xs font-semibold px-2 py-1 rounded-full">
            {car.category}
          </span>
        </div>
        <div className="p-4 space-y-3">
          <h4 className="text-lg font-bold">{car.name} {car.year}</h4>
          <div className="flex items-center space-x-4 text-sm text-gray-600">
            <span className='flex items-center contect-center'>{getIconComponent('Suitcase')} {car.specs.luggage}</span>
            <span className='flex items-center contect-center'>{getIconComponent('People')} &nbsp; {car.specs.passengers}</span>
            <span className='flex items-center contect-center'>{getIconComponent('Gear')} {car.specs.transmission}</span>
          </div>
          <div className="text-sm space-y-1">
            ...
            <p>
              <strong>Deposit:</strong> {car.deposit.toLocaleString()} AED
            </p>
            <p>
              <strong>Rent Charges:</strong> {car.rent.toLocaleString()} AED + VAT@5
            </p>
          </div>
          <button
            onClick={() => handleSelect(car)}
            className={`w-full font-semibold py-2 rounded-lg transition-colors ${
              isSelected ? 'bg-blue-600 text-white hover:bg-blue-700' : 'bg-black text-white hover:bg-gray-800'
            }`}
          >
  ...
  return (
    <div className="bg-white rounded-2xl shadow-xl p-8 space-y-8">
      <div>
        <h2 className="text-2xl font-bold text-gray-900">We know the importance of your Priority</h2>
        <p className="mt-2 text-gray-600">
          You have selected vehicle(s), in case the selected vehicle(s) are not available we make sure to provide you with
          a car from the given priorities. Please select any vehicles from the following list of cars, telling us your
          priorities will help us to serve you better.
        </p>
      </div>
      <div className="space-y-4">
        <h3 className="text-xl font-bold text-gray-800">More Booking options from the same Vendor(s)</h3>
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
          {sameVendorCars.map(car => (
            <CarOptionCard key={`${car.name}-${car.year}`} car={car} />
          ))}
        </div>
      </div>
      <div className="space-y-4">
        <h3 className="text-xl font-bold text-gray-800">More Booking options from the different Vendor(s)</h3>
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
          {differentVendorCars.map(car => (
            <CarOptionCard key={`${car.name}-${car.year}`} car={car} />
          ))}
        </div>
      </div>
      <div className="pt-6 border-t">
        <label className="flex items-center contect-center">
          <input type="checkbox" className="h-5 w-5 rounded border-gray-300 text-blue-600 focus:ring-blue-500"  style={{transform: 'translateY(3px)'}} />
          <span className="ml-3 text-gray-700">Yes, I understand and read the Priority Statement.</span>
        </label>
      </div>
    </div>
  );
                ...
                <div className="border-t pt-8 mt-8 grid grid-cols-1 md:grid-cols-3 gap-6 items-center">
                    ...
                    <div className="md:col-span-2 space-y-3">
                        ...
                        <div className="space-y-2">
                            <label className="flex items-center text-sm"><input type="checkbox" className="h-4 w-4 rounded border-gray-300 text-blue-600 focus:ring-blue-500" style={{marginRight: '4px',transform: 'translateY(2px)'}} />I have read and accepted the <a href="#" className="text-blue-600 hover:underline ml-1">Terms of Use</a> of caryaati.com</label>
                            <label className="flex items-center text-sm"><input type="checkbox" className="h-4 w-4 rounded border-gray-300 text-blue-600 focus:ring-blue-500 mr-2" style={{marginRight: '4px',transform: 'translateY(2px)'}} />I have read the accepted the <a href="#" className="text-blue-600 hover:underline ml-1">Terms & Conditions</a> of Rent a Car Supplier</label>
                            <label className="flex items-center text-sm"><input type="checkbox" className="h-4 w-4 rounded border-gray-300 text-blue-600 focus:ring-blue-500 mr-2" style={{marginRight: '4px',transform: 'translateY(2px)'}} />I have read and accept <a href="#" className="text-blue-600 hover:underline ml-1">Privacy Policy and Cookie Policy</a></label>
                        </div>
                    </div>
                </div>
            </form>
...
const BookingPage: React.FC = () => {
    ...
    const handleEditDateTime = () => {
        ...
        // If the date picker is in Step 1, ensure the user is on that step
        setCurrentStep(1); // Adjust if the picker is elsewhere
    };
            ...
            <div className="lg:w-2/5 bg-white rounded-2xl p-6 shadow-xl flex flex-col">
                <div>
                    ...
                    <CustomDateRangePicker
                        ...
                        isCarCard={false} // Adjust based on your context
                ...
                <div className="mt-auto pt-6">
                    ...
                    <button onClick={nextStep} className="w-full bg-blue-600 hover:bg-blue-700 text-white font-bold py-3 px-4 rounded-xl transition-transform transform hover:scale-105 shadow-lg" style={{ borderRadius: '8px' }}>Next</button>
                </div>
            </div>
        </div>
    );
                    ...
                    <div className="mt-10">
                        ...
                        {currentStep === 6 && (
                            <div className="bg-white rounded-2xl shadow-xl p-8">
                                <div className="flex justify-end space-x-4 mb-8">
                                    ...
                                    <Link href="/customer-dashboard/index" className="bg-orange-400 text-white font-semibold px-6 py-2 rounded-lg hover:bg-orange-500 transition-colors">Dashboard</Link>
                                </div>
                        ...
                        {currentStep < totalSteps && (
                            <div className="flex justify-between items-center mt-10">
                                <button onClick={prevStep} disabled={currentStep === 1} style={{ borderRadius: '8px' }} className="py-2.5 px-6 rounded-xl hover:bg-gray-200 font-medium disabled:opacity-50">Previous</button>
                                <button onClick={nextStep} style={{ borderRadius: '8px' }} className="bg-blue-600 hover:bg-blue-700 text-white font-medium py-2.5 px-8 rounded-xl shadow-md">{currentStep === totalSteps - 1 ? 'Finish Booking' : 'Next'}</button>
                            </div>
                        )}
                    </div>
                </div>
