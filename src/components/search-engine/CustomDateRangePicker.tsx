...
const CustomDateRangePicker: React.FC<CustomDateRangePickerProps> = ({ value, onChange, placeholder, isCarCard }) => {
  ...
  const handleDateClick = (day: number) => {
    ...
    if (!startDate || (startDate && endDate)) {
      ...
      setEndDate(null);
      onChange(newStartDate, null); // Notify parent of start date change
  ...
  const handleTimeChange = (field: 'start' | 'end', hours12: number, minutes: number, period: 'AM' | 'PM') => {
    ...
    if (newDate) {
      ...
      if (field === 'start') {
        ...
        onChange(new Date(newDate), endDate); // Notify parent of start date change
      } else {
        setEndDate(new Date(newDate));
        onChange(startDate, new Date(newDate)); // Notify parent of end date change
      }
    }
  };
  ...
  const formatTime = (date: Date | null) => {
    return date ? format(date, 'hh:mm aa') : '12:00 AM';
  };

  const formatInputValue = () => {
    if (startDate && endDate) {
      return `${format(startDate, 'MM/dd/yyyy hh:mm aa')} - ${format(endDate, 'MM/dd/yyyy hh:mm aa')}`;
    } else if (startDate) {
      return `${format(startDate, 'MM/dd/yyyy hh:mm aa')} - Select end date`;
    }
    return '';
  };
    ...
    <div className={`custom-date-range-picker max-[340px]:w-[95vw] `} ref={wrapperRef}>
      ...
      {isOpen && (
        <div className={`calendar-container ${carCard ? 'z-1' : 'z-999'}`}>
          ...
          <button className="apply-button" onClick={() => { onChange(startDate, endDate); setIsOpen(false); }}>
            Apply
          </button>
        </div>
      )}
